...
arsenalReserve  = {
     ...
     1005   : [ 'Thomas Partey'      , 'DMF', 35,  25000000],
...
listFinance = [
        ['Balance'                  , 5000000000],
        ['transferBudgetSeason'     ,  100000000],
        ['transferBudgetRemaining'  ,  100000000],
...
def buyPlayers():
    while True:
        try:
            ...
            if  choice_4a.isdigit() == True and int(choice_4a) in playerListed and int(choice_4a) in arsenalAll:
                ...
                pagar()
                continue
            elif choice_4a.isdigit() == True and int(choice_4a) in playerListed and int(choice_4a) not in arsenalAll:
                choice_4a = int(choice_4a)
                print('    \n\t| Number|\tPlayer Name  \t| Pos\t| Age |\t Value  ')
                print (f'        |  {choice_4a} | {playerListed[choice_4a][0].ljust(15)}\t|  {playerListed[choice_4a][1]}\t| {playerListed[choice_4a][2]}  | {playerListed[choice_4a][3]:,}')
            else:
            ...
            while True:
                ...
                if playerListed[choice_4a][3] <= (listFinance[2][1]) and choice_4b <= (playerListed[choice_4a][3]*0.9):
                    print('    \n\t| Number|\tPlayer Name  \t| Pos\t| Age |\t Value  ')
                    print(f'        |  {choice_4a} | {playerListed[choice_4a][0].ljust(15)}\t|  {playerListed[choice_4a][1]}\t| {playerListed[choice_4a][2]}  | {playerListed[choice_4a][3]:,}')
...
def sureUnsureRetire():
    while True:
        sure = input("\n    Are you sure you want to retire this players 'YES / NO' : ").lower()
        if sure in macamYes:
            print("\n\t\t DONE, successful processed !")
            pagar()
            break
...
def retire():
    while True:
        try:
            arsenal_All()
            choice_5c   = input("\nInsert the players number will retire (age > 32): ")
            ...
            if  int(choice_5c) in arsenalAll:
                ...
                if arsenalAll[choice_5c][2] >= 32:
                    ...
                    playersRetire.clear()
                    del arsenalAll[choice_5c]
                    sureUnsureRetire()
                    break
                else:
                    print('\n\tPlayers still young, its not time to retire yet !')
                    pagar()
            else:
                print("\n\t\t-- Players name not found --\n")
                print("\t\t     Insert Correctly !!")
        ...
        try:
            ...
            while True:
                ...
                if user == 1:
                    ...
                    arsenalAll[choice_6f][0] = (username)
                    break
                elif user == 3:
                    userpos    = input('Change players position             : ').upper()
                    arsenalAll[choice_6f][1] = (userpos)
                    break
                elif user == 4:
                    userage    = int(input('Change players age                  : '))
                    arsenalAll[choice_6f][2] = (userage)
                    break
                elif user == 5:
                    uservalue  = int(input('Change players value                : '))
                    arsenalAll[choice_6f][3] = (uservalue)
                    break
                elif user == 6:
                    break
            sureUnsureUpdate()
        ...
        try:
            ...
            while True:
                ...
                if user == 1:
                    ...
                    username   = input('Change players name                 : ').capitalize()
                    playerListed[choice_6g][0] = (username)
                    break
                elif user == 3:
                    userpos    = input('Change players position             : ').upper()
                    playerListed[choice_6g][1] = (userpos)
                    break
                elif user == 4:
                    userage    = int(input('Change players age                  : '))
                    playerListed[choice_6g][2] = (userage)
                    break
                elif user == 5:
                    uservalue  = int(input('Change players value                : '))
                    playerListed[choice_6g][3] = (uservalue)
                    break
                elif user == 6:
                    break
            sureUnsureUpdate()
    ...
    while True:
        try:
            ...
            print('''
                    ...
                    Item :
                    1. Balance
                    2. Transfer Budget Season 
                    3. Transfer Budget Remaining
                    4. Loan Outstanding
                    5. Total Wage
                    6. Current Wage
                    7. Back to Menu Editor
                ''')
            while True:
                user = int(input("Insert item that you want to update : "))
                # del arsenalAll[choice_6f]
